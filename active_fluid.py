...
class active_fluid:     # OOP
    ...
    def __init__(self,alpha=1, u=10,Fs=100, N_ptcl=40000,N_ens=1, mu=1,Dt = 1,Dr=1):

        self.initial_state = (alpha,u,Fs,N_ptcl,N_ens,mu,Dt,Dr)    # recording initial state
         # coefficients
        self.set_coeff(alpha,u,Fs,N_ptcl,N_ens,mu,Dt,Dr) 
    ...
    def set_coeff(self,alpha=1, u=10,Fs=100, N_ptcl=40000,N_ens=1, mu=1,Dt = 1,Dr=1):
        ...
        self.mode='free'
        self.omega=np.zeros(self.N_ens)
...
#         return np.sum(interact*strength)
    ...
    def force(self):
        ...
        # axis 0 for passive, axis 1 for active, axis 2 for ensembles
        (rel_x,rel_y) = self.periodic(x-X,y-Y)
        ...
        F_active  = (-np.sum(strengthX,axis=0),-np.sum(strengthY,axis=0))      #sum over bodies, sum over objects
        F_passive = (np.sum(np.sum(strengthX,axis=0),axis=0),np.sum(np.sum(strengthY,axis=0),axis=0))      # sum over bodies, sum over active particles
        torque    = np.sum(np.sum(centerX*strengthY-centerY*strengthX,axis=0),axis=0)        
...
#         return (F_active, torque)     # F_active ~ -partialV
    ...
    def set_zero(self):              # initializing simulation configurations
        ...
        self.x = np.random.uniform(self.R*2, self.L-self.R*2,(1,self.N_ptcl,self.N_ens))     # starting with uniformly distributed particles
        self.y = np.random.uniform(self.R*2, self.L-self.R*2,(1,self.N_ptcl,self.N_ens))
        self.theta = np.random.uniform(-np.pi/2, np.pi/2,(1,self.N_ptcl,self.N_ens))

        # self.X = np.array([-self.l_passive/2,self.l_passive/2])
        # self.Y = np.array([0,0])     
        self.X = np.zeros((self.N_ens))
        self.Y = np.zeros((self.N_ens))  
        
        self.Theta = np.random.uniform(-np.pi, np.pi,(self.N_ens))

    def tumble(self):             # random part of s dynamics
        tumble = np.random.choice([0,1], (self.N_ptcl,self.N_ens), p = [1-self.dt*self.alpha, self.dt*self.alpha]) # 0 no tumble, 1 tumble
        return tumble
...
#         F_active,torque = self.force()
        ...
        self.theta       += np.sqrt(2*self.Dr*self.dt)*np.random.normal(0,1,(self.N_ptcl,self.N_ens))     # ABP noise
        self.x           +=  self.dt*(self.u*(np.cos(self.theta))+self.mu*F_active[0])       # deterministic
        self.x           +=  np.sqrt(2*self.Dt*self.dt)*np.random.normal(0,1,(self.N_ptcl,self.N_ens))    # thermal noise
        self.y           +=  self.dt*(self.u*(np.sin(self.theta))+self.mu*F_active[1])       # deterministic
        self.y           +=  np.sqrt(2*self.Dt*self.dt)*np.random.normal(0,1,(self.N_ptcl,self.N_ens))    # thermal noise

        # passive object moving
#         print(type(self.X))
#         self.X           += self.dt*self.mu_T*F_passive[0]
#         self.Y           += self.dt*self.mu_T*F_passive[1]
        if self.mode=='free':
            self.omega += (1/self.I_R)*self.dt*torque
...
#         Y6 = self.R*np.sin(thetas+np.pi)-RA*np.sin(self.Theta.reshape(-1,1)+np.pi)
        ...
        #GEAR 3
        thetas = np.linspace(-np.pi/6,np.pi/6,self.N_body).reshape(-1,1,1)+self.Theta
        RA = np.array(self.R)
        
        X1 = -RA*np.cos(thetas)  + 2/np.sqrt(3)*self.R*np.cos(self.Theta)
        Y1 = -RA*np.sin(thetas)  + 2/np.sqrt(3)*self.R*np.sin(self.Theta)
        X2 = -RA*np.cos(thetas+np.pi*2/3) + 2/np.sqrt(3)*self.R*np.cos(self.Theta+np.pi*2/3)
        Y2 = -RA*np.sin(thetas+np.pi*2/3) + 2/np.sqrt(3)*self.R*np.sin(self.Theta+np.pi*2/3)
        X3 = -RA*np.cos(thetas-np.pi*2/3) + 2/np.sqrt(3)*self.R*np.cos(self.Theta-np.pi*2/3)
        Y3 = -RA*np.sin(thetas-np.pi*2/3) + 2/np.sqrt(3)*self.R*np.sin(self.Theta-np.pi*2/3)
        centerX = np.concatenate([X1,X2,X3],axis=0)
        centerY = np.concatenate([Y1,Y2,Y3],axis=0)
